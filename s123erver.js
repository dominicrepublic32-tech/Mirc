...
// Simple HTTP Basic Auth gate in front of the entire app — dashboard, every
// /api/* route, the SSE stream, and the OAuth /callback — so nobody but you
// can open or control this bot. Username is ignored; only the password is
// checked. Override via the BOT_PASSWORD env var if you ever want to change
// it without touching code. Nothing below this block is modified.
...
app.use((req, res, next) => {
  ...
  res.set('WWW-Authenticate', 'Basic realm="Zone Touch 10-in-1"');
  res.status(401).send('Authentication required');
});

const PORT         = process.env.PORT         || 3000;
const APP_ID_LIVE  = process.env.DERIV_APP_ID || '33NVjvYI37KDCUslgcIgi';
const REDIRECT_URI = process.env.REDIRECT_URI || 'https://mirc.onrender.com/callback';
