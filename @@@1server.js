...
// Whether ANY enabled structure feature (pause and/or precheck, for the given
...
function structureNeedsTf(cfg, logic, tf){
  ...
  if (!pauseOn && !precheckOn) return false;
  ...
  return pauseTfs.includes(tf) || precheckTfs.includes(tf);
}
...
function structureTrendConfirmed(slot, logic, variant, requiredTrend){
  ...
  const trends = timeframes.map(tf => detectSwingTrend(structBufFor(slot, tf), strength, confirmCount));
