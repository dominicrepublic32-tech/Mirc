...
class Account {
  ...
  openCandleStream(symbol, cb){
    const ws = new WebSocket(WS_DEMO);
    ws.on('open', () => ws.send(JSON.stringify({ ticks_history: symbol, adjust_start_time: 1, count: 50, end: 'latest', granularity: 60, style: 'candles', subscribe: 1 })));
    ws.on('message', raw => {
      let m; try { m = JSON.parse(raw); } catch { return; }
...
function defaultCfg(){
  return {
    symbol:'1HZ100V', logic:1, observe_mode:'ticks', observe_count:5, reaction_range:0.3,
...
class Slot {
  ...
  pushState(){
    push({ type:'state', slotId:this.id, data:{ running:this.running, busy:this.busy, stats:this.stats, phase:this.phase, liveCount:this.liveCount, sessionProfit:this.sessionProfit } });
  }
  ...
  softReset(){ this.phase='warmup'; this.zoneHigh=null; this.zoneLow=null; this.confSide=null; this.confBuf=[]; this.buf=[]; this.liveCount=0; }
  snap(){ return { id:this.id, idx:this.idx, running:this.running, busy:this.busy, stats:this.stats, cfg:this.cfg, phase:this.phase, liveCount:this.liveCount, sessionProfit:this.sessionProfit }; }
}
...
// ─── logic 1 — zone touch, fire both barriers ──────────────────────────────────
function fireBoth(slot){
  const c = slot.cfg;
  const pHigh = buildParams(c.symbol,'ONETOUCH',c.barrier1,c.duration_value,c.duration_unit,c.stake);
  const pLow  = buildParams(c.symbol,'ONETOUCH',c.barrier2,c.duration_value,c.duration_unit,c.stake);
  Promise.all([
    runContract(slot,pHigh,'ZONE +barrier').catch(e=>{slot.emit('Error: '+e.message,'err');return null;}),
    runContract(slot,pLow ,'ZONE -barrier').catch(e=>{slot.emit('Error: '+e.message,'err');return null;}),
  ]).then(r=>afterTrade(slot, r.some(x=>x&&x.won)));
}

function evalL1(slot){
  ...
  slot.busy=true; slot.buf=[]; slot.liveCount=0; slot.pushState();
  fireBoth(slot);
}

// ─── logic 2 & 3 — reversal touch ──────────────────────────────────────────────
function evalL23_watch(slot){
  if (slot.busy || slot.buf.length < slot.cfg.observe_count) return;
...
function fireReversal(slot, side){
  ...
  // ── Logic 2: single ONETOUCH ──
  slot.emit(`Confirmed — ONETOUCH @${barrier} | ${side.toUpperCase()} reversal`);
  const p = buildParams(c.symbol,'ONETOUCH',barrier,c.duration_value,c.duration_unit,c.stake);
  runContract(slot, p, `ONETOUCH ${side.toUpperCase()}`, { role:'primary' })
    .then(rec => afterTrade(slot, rec.won))
    .catch(e => { slot.emit('Trade error: '+e.message,'err'); afterTrade(slot,false); });
}
...
function dispatch(slot, val){
  ...
  if (slot.phase === 'trading' || slot.busy) return;     // never act while a trade is live
  ...
  if (slot.liveCount < slot.cfg.observe_count) {
    ...
    slot.pushState(); return;                            // STILL WARMING UP — no evaluation, no trade
  }
  ...
  if (slot.cfg.logic === 1) evalL1(slot);
  else if (slot.phase === 'watching') evalL23_watch(slot);
}
...
function startWatcher(slot){
  closeWs(slot.dataWs); slot.dataWs=null; closeWs(slot.trendWs); slot.trendWs=null;
  const cfg = slot.cfg;
  if (cfg.filter === 'trend') {
  ...
  slot.emit(`Stream open — ${cfg.observe_mode==='candles'?'M1 Candles':'Ticks'} | Logic ${cfg.logic} | Filter: ${cfg.filter}`);
}
...
async function startSlot(slot){
  ...
  slot.emit(`Bot started — collecting fresh data 0/${slot.cfg.observe_count} (no trade until warm-up done)…`);
  return { success:true };
}
...
setInterval(() => {
  ...
  slots.forEach(slot => {
    ...
    if (!ws || ws.readyState === WebSocket.CLOSED || ws.readyState === WebSocket.CLOSING) {
      ...
      slot.buf=[]; slot.liveCount=0; slot.phase='warmup'; startWatcher(slot);
    }
  });
}, 10000);
...
// ─── AI analyst (Claude) ────────────────────────────────────────────────────────
function buildAnalytics(slot){
  const h = slot.history;
  const summary = {
    symbol:slot.cfg.symbol, logic:slot.cfg.logic, running:slot.running,
    trades:slot.stats.trades, wins:slot.stats.wins, losses:slot.stats.losses,
    winRate: slot.stats.trades ? +(slot.stats.wins/slot.stats.trades*100).toFixed(1) : 0,
    profit:slot.stats.profit, sessionProfit:slot.sessionProfit,
  };
  let analysed=0, nearMiss=0, wrongDirection=0;
  h.forEach(r => {
    if (r.won) return;
    if (r.type==='ONETOUCH' && r.barrierAbs!=null && r.entrySpot!=null && r.maxSpot!=null && r.minSpot!=null) {
      analysed++;
      const upTarget = r.barrierAbs > r.entrySpot;
      const closest  = upTarget ? r.maxSpot : r.minSpot;
      const dist     = Math.abs(r.barrierAbs - closest);
      const full     = Math.abs(r.barrierAbs - r.entrySpot) || 1;
      if (dist/full <= 0.2) nearMiss++;                          // came within 20% of barrier
      const moved = upTarget ? (r.maxSpot > r.entrySpot) : (r.minSpot < r.entrySpot);
      if (!moved) wrongDirection++;                              // never moved toward target
    }
  });
  return { ...summary, oneTouchLossAnalysed:analysed, nearMissLosses:nearMiss, wrongDirectionLosses:wrongDirection };
}

app.post('/api/agent', async (req, res) => {
  try {
    const key = process.env.ANTHROPIC_API_KEY;
    if (!key) return res.json({ error:'ANTHROPIC_API_KEY is not set on the server (add it in Render → Environment).' });
    const { question, slotId } = req.body || {};
    if (!question) return res.json({ error:'No question provided' });

    const scope = (slotId && slotOf.get(slotId)) ? [slotOf.get(slotId)] : slots;
    const context = scope.map(s => ({
      id:s.id, cfg:s.cfg, running:s.running,
      analytics:buildAnalytics(s),
      recentTrades:s.history.slice(0,40),
      recentLogs:s.logs.slice(-40).map(l => ({ t:l.time, lvl:l.level, m:l.msg })),
    }));

    const sys = `You are the trading-analyst AI built into "Zone Touch 10-in-1", a Deriv synthetic-indices bot.
LOGICS:
- Logic 1 (Zone Touch): when price compresses into a tight zone, it fires BOTH a +barrier and -barrier One Touch at once.
- Logic 2 (Reversal Touch): locks a zone, waits for price to TOUCH the zone HIGH or LOW, then fires a single ONETOUCH on that side.
- Logic 3 (Reversal Combo): same trigger as Logic 2, but each side's PRIMARY ONETOUCH may carry ONE attached contract (Rise/Fall/Higher/Lower/Touch/NoTouch/Vanilla) fired the same instant. Win/rest decisions use the PRIMARY one touch only.
TRADE RECORD FIELDS: entrySpot, exitSpot, barrierAbs (absolute barrier price), maxSpot/minSpot (highest/lowest price reached during the contract), role ('primary' or 'combo').
Use maxSpot/minSpot vs barrierAbs to judge "near miss" (price came close to the barrier) and "wrong direction" (price never moved toward the target). Pre-computed counts are in analytics.
Answer the user's question precisely from the JSON. Do real math. Be concise, specific and practical. For recommendations (barrier, stake, options, market) justify with the actual numbers.`;
    ...
    const r = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body:JSON.stringify({ model:CLAUDE_MODEL, max_tokens:1200, system:sys, messages:[{ role:'user', content:userMsg }] }),
    });
    const data = await r.json();
    if (data.error) return res.json({ error:data.error.message || 'Claude API error' });
...
app.post('/api/slot/:id/cfg', (req, res) => { const s=slotOf.get(req.params.id); if (!s) return res.status(404).json({ error:'Not found' }); s.cfg={ ...s.cfg, ...req.body }; persistSoon(); res.json(s.cfg); });
