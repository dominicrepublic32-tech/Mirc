...
const APP_ID_LIVE  = process.env.DERIV_APP_ID || '33Ou3L67dWp4J0lBqgVYF';
const REDIRECT_URI = process.env.REDIRECT_URI || 'https://diamond-vdtb.onrender.com/callback';
...
async function runContract(slot, params, label, opts = {}){
  ...
  // passes this in when Safe Stake tracking applies to this trade. Not
  // supported in backtest (see superCheckSafeStake for why) — ssCtrl is
  // simply never created/passed there, so this whole block is a no-op and
  // backtest behaves exactly as before.
  const ssCtrl = !backtest ? (opts.safeStakeController || null) : null;
  let cid, settled;
  if (backtest) {
  ...
  // Counterfactual tracking (added) — for a 'saved' trade, keep watching real
  // market price in the background until the ORIGINAL duration would have
  // naturally expired, then work out whether that trade would have won or
  // lost if left alone, and retroactively tag rec.wouldHaveWon so the
  // history can show it correctly (red dot = would've lost, saving it was
  // right; green dot = would've won, the save gave up a real profit).
  if (saved && ssCtrl) shadowTrackSavedTrade(slot, rec, ssCtrl);
...
function structureNeedsTf(cfg, logic, tf){
  ...
  if (!pauseOn && !precheckOn && !efficiencyOn) return false;
  ...
  return pauseTfs.includes(tf) || precheckTfs.includes(tf) || efficiencyTf === tf;
}
...
//
// NOT available in backtest: backtest resolves each Super Logic trade
// instantly via a forward-scan through history the moment it fires (see
// runContractBacktestResolve), rather than progressing tick-by-tick in
// lockstep with the rest of the replay — so by the time an older trade
// would need protecting, the newer trades that should trigger that
// protection haven't been "fired" yet in the replay's own timeline. Making
// this work in backtest would need a deeper rework of how Super Logic trades
// resolve there. superOpenPositions is simply never populated during
// backtest (see superFireTrade), so this function has nothing to check and
// is effectively a no-op — backtest behavior is otherwise unaffected.
...
function superFireTrade(slot, direction){
  ...
  if (concurrent) {
    ...
    // Safe Stake (added) — only tracks Rise/Fall trades, only outside
    // backtest (see superCheckSafeStake for why), and only when the feature
    // itself is on. ssCtrl IS the tracked position record — runContract
    // fills in entrySpot/contractId on it as they become known, and removes
    // it from slot.superOpenPositions once the trade settles.
    let ssCtrl = null;
    if (useRiseFall && !slot.backtest && slot.cfg.super_safestake_enabled) {
      ssCtrl = {
        direction, entrySpot: null, contractId: null, closeRequested: false,
        fireEpoch, durationSeconds: superDurationToSeconds(c.duration_value, c.duration_unit),
        symbol: c.symbol,
        requestClose(){
          if (this.closeRequested) return;
          this.closeRequested = true;
          if (!slot.cfg.demo_mode && this.contractId) acct.sell(this.contractId).catch(() => {});
        },
      };
