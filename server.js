...
function defaultCfg(){
  return {
    ...
    // Uses the same timeframe/swing settings above. Off by default.
    ...
    // above, for Super Logic. Off by default; independent of the after-loss pause.
...
class BacktestRunner {
  ...
  async run() {
    try {
      ...
      // features are even on. Structure Pre-Check (added) uses the exact same
      // buffers/timeframe setting, so it's folded in here too.
      const structureLogic = cfg.logic === 'super' ? 'super' : 'golden';
      const structurePauseOn = !!cfg[`${structureLogic}_structure_pause_enabled`] || !!cfg[`${structureLogic}_structure_precheck_enabled`];
...
// right before every trade fires instead of only after a losing streak.
// Either, both, or neither can be enabled at once — they don't interact.
function structurePrecheckOk(slot, logic){
  const cfg = slot.cfg;
  if (!cfg[`${logic}_structure_precheck_enabled`]) return true;
  const label = logic === 'golden' ? 'Golden' : 'Super';
  const ok = structureTrendConfirmed(slot, logic);
  if (!ok) {
...
function startWatcher(slot){
  ...
  // after-loss pause and the added before-trade precheck) — one shared
  // subscription for all of them.
  const goldenStructureOn = cfg.golden_structure_pause_enabled || cfg.golden_structure_precheck_enabled;
  if (cfg.filter === 'trend' || cfg.golden_trend_enabled || cfg.golden_consolidation_filter_enabled || (goldenStructureOn && cfg.golden_structure_pause_timeframes !== 'm5')) {
  ...
  if (cfg.golden_trend_enabled || (goldenStructureOn && cfg.golden_structure_pause_timeframes !== 'm1')) {
