...
class BacktestRunner {
  ...
  async run() {
    try {
      ...
      // features are even on. Structure Pre-Check (added) has its OWN separate
      // timeframe setting (not shared with the after-loss pause), so each is
      ...
      const structurePauseTf    = cfg[`${structureLogic}_structure_pause_timeframes`] || 'm1';
      const structurePrecheckOn = !!cfg[`${structureLogic}_structure_precheck_enabled`];
      const structurePrecheckTf = cfg[`${structureLogic}_structure_precheck_timeframes`] || 'm1';
      const structureWantsM1 = (structurePauseOn && structurePauseTf !== 'm5') || (structurePrecheckOn && structurePrecheckTf !== 'm5');
      const structureWantsM5 = (structurePauseOn && structurePauseTf !== 'm1') || (structurePrecheckOn && structurePrecheckTf !== 'm1');
...
function fireReversal(slot, side){
  ...
  // Pause; if enabled, verifies market structure isn't ranging right now,
  // before this trade ever fires.
  if (!structurePrecheckOk(slot, 'golden')) {
    slot.phase='watching'; slot.zoneHigh=null; slot.zoneLow=null; slot.confSide=null; slot.confBuf=[];
    slot.pushState();
    return;
  }
...
function structureTrendConfirmed(slot, logic, variant){
  const cfg = slot.cfg;
  const isPrecheck = variant === 'precheck';
  const strength     = isPrecheck ? cfg[`${logic}_structure_precheck_swing_strength`]       : cfg[`${logic}_structure_swing_strength`];
  const confirmCount = isPrecheck ? cfg[`${logic}_structure_precheck_swing_confirm_count`]  : cfg[`${logic}_structure_swing_confirm_count`];
  const timeframes   = (isPrecheck ? cfg[`${logic}_structure_precheck_timeframes`] : cfg[`${logic}_structure_pause_timeframes`]) || 'm1';
...
function structurePrecheckOk(slot, logic){
  const cfg = slot.cfg;
  if (!cfg[`${logic}_structure_precheck_enabled`]) return true;
  const label = logic === 'golden' ? 'Golden' : 'Super';
  const ok = structureTrendConfirmed(slot, logic, 'precheck');
  if (!ok) {
    slot.emit(`${label}: structure pre-check — market structure is ranging (no confirmed higher-highs/lows or lower-highs/lows) — skipping trade`, 'err');
  }
  return ok;
}
...
function superOnCandle(slot, candle){
  ...
  slot.emit(`Super: ✓ ${sameColor.toUpperCase()}x${n} + ${oppColor.toUpperCase()}x${m} → ${direction.toUpperCase()} setup — checking momentum…`);
  ...
  // Pause; if enabled, verifies market structure isn't ranging right now,
  // before this trade ever fires.
  if (!structurePrecheckOk(slot, 'super')){
    slot.superRun = []; // discard all candles from this setup
    return;
  }
...
function startWatcher(slot){
  ...
  // independent timeframe setting) — one shared subscription for all of them.
  const goldenStructureWantsM1 = (cfg.golden_structure_pause_enabled && cfg.golden_structure_pause_timeframes !== 'm5')
    || (cfg.golden_structure_precheck_enabled && cfg.golden_structure_precheck_timeframes !== 'm5');
  const goldenStructureWantsM5 = (cfg.golden_structure_pause_enabled && cfg.golden_structure_pause_timeframes !== 'm1')
    || (cfg.golden_structure_precheck_enabled && cfg.golden_structure_precheck_timeframes !== 'm1');
  ...
  // when either check is set to watch M5 — mirrors Super Logic's setup.
